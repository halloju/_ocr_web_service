...
import SideBarEntryGeneral from '@/components/SideBarEntryGeneral.vue';
...
export default {
    ...
    methods: {
        ...
        formSubmitted(shape) {
            // save correct color
            console.log('form submitted', shape);
            const idx = this.shapes.findIndex((r) => r.name === shape.name);
            this.shapes[idx].fill = this.getFillColorByRectangleType(this.shapes[idx].rectangleType);
            // callback/persist
            this.shapesUpdated(shape, idx);
        },
        ...
        // callback on update
        shapesUpdated(shape, idx) {
            if (this.callback && typeof this.callback === 'function') {
                console.log('callback', shape);
                this.callback(shape, idx, this.image_cv_id);
            }
            // save to local storage, if defined
            if (this.localStorageKey) {
                console.log('save to local storage', this.shapes);
