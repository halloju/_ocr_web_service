...
def get_available_templates(db: Session, user_id: str):
    ...
    """
    try:
        available_templates = db.query(TemplateInfo).filter(TemplateInfo.user_id == user_id)
...
def get_template_detail(db: Session, template_id: str):
    ...
    try:
        ...
        # Step 2. 從 DB 拉座標點等資訊
        template_detail = db.query(TemplateInfo).filter(TemplateInfo.template_id == template_id)
