...
class PredictTask(Task):
    ...
    def predict(self, image_id, action, input_params):
        try:
            ...
            logger.error({'predict': {'error_msg': str(
                e), 'image_id': image_id, 'action': action, 'input+params': input_params}})
        ...
        logger.info({'predict': {'image_id': image_id,
                    'data_pred': list(data_pred.keys())}})
...
def upload_to_redis(image_data):
    ...
    try:
        ...
        logger.error(
            {'upload_to_redis': {'error_msg': str(e), 'image_id': image_id}})
...
def get_from_redis(task_id):
    try:
        ...
        if task_result is None:
            logger.error(
                {'get_from_redis': f'task_result of id {task_id} is None'})
            return {'status': 'ERROR', 'status_msg': '5003'}
        task_result_dict = json.loads(task_result.decode('utf-8'))
        return task_result_dict["result"]
    except Exception as e:
        logger.error(
            {'get_from_redis': {'error_msg': str(e), 'task_id': task_id}})
        return {'status': 'FAIL', 'status_msg': '5002'}


def get_predict_data(data_pred, action):
    if action in ['check_front', 'check_back']:
        data_pred['data_results'] = [{'tag': key, 'text': value} for key, value in data_pred.items(
        ) if key not in ['status_code', 'status_msg', 'err_detail']]
    elif action == 'remittance':
        data_pred['data_results'] = [{'tag': key, 'text': value, 'points': remittance_points[key]}
                                     for key, value in data_pred.items() if key not in ['status_code', 'status_msg', 'err_detail']]
    return data_pred
...
def predict_image(self, image_id, action, input_params, start_time):
    ...
    file_name = celery.backend.get(
        get_redis_filename(image_id)).decode("utf-8")
    start_time = datetime.strptime(start_time, "%Y-%m-%d %H:%M:%S")
    try:
        response = self.predict(image_id, action=action,
                                input_params=input_params)
        data_pred = get_predict_data(response, action)
        logger.info({'predict_image': {'task_id': self.request.id, 'process_time': (datetime.now(
        ) - start_time).microseconds, 'action': action, 'status': 'SUCCESS', 'status_msg': ''}})
        return {'status': 'SUCCESS', 'status_msg': '', 'result': data_pred, 'file_name': file_name}
    except MlaasRequestError as e:
        logger.info({'predict_image': {'task_id': self.request.id, 'process_time': (datetime.now(
        ) - start_time).microseconds, 'action': action, 'status': 'FAIL', 'status_msg': e.mlaas_code}})
        return {'status': 'FAIL', 'status_msg': e.mlaas_code, 'file_name': file_name}
    except Exception as e:
        logger.error({'predict_image': {'task_id': self.request.id, 'process_time': (datetime.now(
        ) - start_time).microseconds, 'action': action, 'status': 'FAIL', 'status_msg': '5001', 'error_msg': str(e)}})
